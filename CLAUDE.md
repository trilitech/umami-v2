...
Umami is a Tezos wallet by Trilitech. pnpm + Turborepo monorepo: shared `@umami/*` libraries in `packages/`, deployable apps in `apps/` (Electron desktop, web SPA, Expo mobile, embeddable iframe, static OAuth redirect page). Node `>=22 <23`, pnpm `>=9.7`.
...
cd packages/core && pnpm test -- --testPathPattern=Account.test.ts   # single file (note the --)
cd packages/core && pnpm test:watch        # watch mode (DEV=true, no coverage)
pnpm run ci                      # check-types + lint:ci + format:ci + check-circular-deps
...
- Taquito tracks the current protocol (v25 = Ushuaia/025) — keep `@taquito/*` versions identical across packages, and keep `bignumber.js` on the same major as taquito's to preserve type identity.
...
- Default networks are mainnet + shadownet (ghostnet was sunset May 2026; `GHOSTNET` remains exported only as a test fixture). Changing default networks requires a redux-persist migration in `packages/state/src/migrations.ts`.
