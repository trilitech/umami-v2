...
describe("<RestoreBackupFile />", () => {
  ...
  describe("v21 backups", () => {
    ...
    it("restores from a v21 backup file", async () => {
      ...
      expect(window.localStorage.setItem).toHaveBeenCalledWith("persist:root", expect.any(String));
    });
  });
});
...
describe("useDownloadBackupFile", () => {
  ...
  beforeEach(() => {
    delete (window as any).localStorage;
    window.localStorage = { getItem: (data: string) => data } as any;
  });
  ...
  it("fetches an encrypted state backup", async () => {
    ...
    expect(decrypted).toEqual(
      '{"persist:accounts":"persist:accounts","persist:root":"persist:root"}'
    );
  });
});
