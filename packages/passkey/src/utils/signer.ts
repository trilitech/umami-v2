...
import { b58cencode, prefix, buf2hex } from "@taquito/utils";
...
export class PasskeySigner implements Signer {
  ...
  async sign(bytes: string, magicBytes?: Uint8Array): Promise<{ bytes: string; sig: string; prefixSig: string; sbytes: string }> {
    try {
      ...
      const challengeBuffer = this.hexToArrayBuffer(bytesToSign);
      ...
      // Parse the DER signature using asn1js
      const { r, s } = this.parseDERSignature(authResponse.signature);
  ...
  // Parse DER signature using asn1js
  private parseDERSignature(derSignature: ArrayBuffer): { r: Buffer, s: Buffer } {
    try {
      // Parse the DER-encoded signature
      const asn1 = asn1js.fromBER(derSignature);
