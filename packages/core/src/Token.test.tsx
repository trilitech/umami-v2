import { type RawTzktTokenMetadata } from "@umami/tzkt";
...
describe("fromRaw", () => {
  test("fa1.2", () => {
    const result = fromRawToken(fa1Token.token);
    const expected = {
      type: "fa1.2",
  ...
  describe("fa2", () => {
    test("valid fa2 token", () => {
      const result = fromRawToken(fa2Token.token);
    ...
    test("invalid fa2 token (missing tokenId)", () => {
      ...
      const result = fromRawToken(token);
  ...
  describe("NFT", () => {
    it("returns FA2 token if decimals field is present", () => {
      const result = fromRawToken({ ...nft.token, metadata: { decimals: "5" } });
      expect(result).toEqual({
        type: "fa2",
    ...
    it("parses valid NFT", () => {
      const expected = {
        ...
        id: 10899466223617,
        metadata: nft.token.metadata as RawTzktTokenMetadata,
        totalSupply: "1",
      };

      const { token } = nft;
      const metadata = token.metadata as RawTzktTokenMetadata;
      metadata.decimals = "0";
      expect(fromRawToken(token)).toEqual(expected);
  ...
  test("fa1 token with name symbol and decimals (tzBTC)", () => {
    const result = fromRawToken(tzBtsc(mockImplicitAddress(0)).token);
  ...
  test("fa1 token with name symbol decimals and icon (Hedgehoge)", () => {
    const result = fromRawToken(hedgehoge(mockImplicitAddress(0)).token);
  ...
  test("fa2 token with thumbnailUri (uUSD)", () => {
    const result = fromRawToken(uUSD(mockImplicitAddress(0)).token);
    const expected = {
      type: "fa2",
