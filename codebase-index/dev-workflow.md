...
- Node `>=20 <21`, pnpm `>=9.7.0` (repo pins `pnpm@9.9.0`), turbo installed globally (`npm install turbo --global`)
...
| Full CI check | `pnpm ci` (check-types + lint:ci + format:ci + check-circular-deps) |
...
| `code_format.yaml` | every push | `pnpm ci` |
...
Used by e2e tests: postgres + tezbox sandbox node (tezos v20.3) + nginx CORS proxy (port `2000X`) + TzKT sync + TzKT API (port `500X`), where `X` = `CUCUMBER_WORKER_ID` for parallel workers. Health check waits for the Alice account (`tz1VSUr8...`) to show its initial 50000 tez. Protocol parameters can be overridden in `sandbox-override-parameters.hjson`.
