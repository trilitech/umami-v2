        ...
        try {
          ...
          try {
            if (Object.keys(hashParams).length > 0 && hashParams.state) {
              instanceParams =
                JSON.parse(
                  base64urlLib.decode(decodeURIComponent(decodeURIComponent(hashParams.state)))
                ) || {};
              if (hashParams.error) error = hashParams.error;
            } else if (Object.keys(queryParams).length > 0 && queryParams.state) {
              instanceParams =
                JSON.parse(
                  base64urlLib.decode(decodeURIComponent(decodeURIComponent(queryParams.state)))
                ) || {};
          ...
          const getTargetOrigin = () => {
            if (typeof instanceParams.redirectToOpener !== "string") {
              return null;
            }
          ...
          if (targetOrigin) {
            // communicate back to a web page through window.postMessage
            const message = {
                channel: "redirect_channel_" + instanceParams.instanceId,
                data: {
                instanceParams: { ...instanceParams, redirectToOpener: true },
            ...
            // communicate back to umami desktop app through a deeplink
            const message = {
              channel: "redirect_channel_" + instanceParams.instanceId,
              ...instanceParams,
              ...hashParams,
            ...
            searchParams = new URLSearchParams(message).toString();
            window.open("umami://auth/" + searchParams, "_self");
          } else {
