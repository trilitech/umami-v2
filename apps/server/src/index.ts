...
router.post('/verify-authentication', async (req, res) => {
  try {
    ...
    const { verified, publicKey, passkeyId, passkeyPublicKey, tezosData } = await verifyAuthentication(123, response);
    res.json({ verified, publicKey, passkeyId, passkeyPublicKey, tezosData });
  } catch (error) {
    console.error(error);
    res.status(500).send('An error occurred');
  }
